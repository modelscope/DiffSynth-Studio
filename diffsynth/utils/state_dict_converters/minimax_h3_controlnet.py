"""Convert PAI's MiniMax-H3 control branch onto this repo's key layout.

Note this converter fuses tensors (three `to_q`/`to_k`/`to_v` rows become one per-head interleaved
`qkv_proj`), so it cannot support `"disk"` offload: `DiskMap` resolves one source tensor per target
key and applies no transform, which would install an unfused weight of the wrong shape. Use the
`"cpu"` offload the low-VRAM script defaults to.
"""
import torch
