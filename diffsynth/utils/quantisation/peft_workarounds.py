...
try:
    from peft.import_utils import is_quanto_available  # type: ignore
except Exception:
...
try:
    # peft 版本里通常有这个全局映射
    from peft.tuners.lora.layer import LORA_MODULES_MAPPING

    if is_quanto_available:
        if QLinear is not None:
            LORA_MODULES_MAPPING[QLinear] = QuantoLoraLinear
        if QConv2d is not None:
            LORA_MODULES_MAPPING[QConv2d] = QuantoLoraConv2d
except Exception:
    # 如果 peft 版本不匹配，就静默跳过，保持现有行为
    pass