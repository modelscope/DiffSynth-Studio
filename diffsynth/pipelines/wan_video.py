...
class WanVideoPipeline(BasePipeline):
    ...
    def __call__(
        ...
        depth_video: Optional[list[Image.Image]] = None,    # MODIFY: depth video
        blending_area_mask: Optional[list[Image.Image]] = None, # MODIFY: blending area mask video
        ...
        inputs_shared = {
            ...
            # MODIFY: add depth video and blending area mask
            "vace_video": vace_video, "vace_video_mask": vace_video_mask, "depth_video": depth_video, "blending_area_mask": blending_area_mask, "vace_reference_image": vace_reference_image, "vace_scale": vace_scale, 
...
class WanVideoUnit_InputVideoEmbedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, input_video, noise, tiled, tile_size, tile_stride, vace_reference_image, framewise_decoding):
        ...
        if vace_reference_image is not None:
            ...
            # MODIFY: multiple reference frames
            bs, c, f, h, w = vace_reference_image.shape
            vace_reference_frames = [vace_reference_image[0, :, j:j+1] for j in range(f)]
            vace_reference_latents = pipe.vae.encode(
                vace_reference_frames,
                device=pipe.device,
                tiled=tiled,
                tile_size=tile_size,
                tile_stride=tile_stride,
            ).to(dtype=pipe.torch_dtype, device=pipe.device)
            vace_reference_latents = torch.concat([u.unsqueeze(0) for u in vace_reference_latents], dim=2)
...
class WanVideoUnit_VACE(PipelineUnit):
    def __init__(self):
        super().__init__(
            # MODIFY: add depth_video and blending_area_mask
            input_params=("vace_video", "vace_video_mask", "depth_video", "blending_area_mask", "vace_reference_image", "vace_scale", "height", "width", "num_frames", "tiled", "tile_size", "tile_stride"),
            output_params=("vace_context", "vace_depth_latents", "vace_scale"),
            onload_model_names=("vae",)
        )
    def process(
        self,
        ...
        # MODIFY: add depth_video and blending_area_mask
        vace_video, vace_video_mask, depth_video, blending_area_mask, vace_reference_image, vace_scale,
        height, width, num_frames,
        tiled, tile_size, tile_stride
    ):
       if vace_video is not None or vace_video_mask is not None or vace_reference_image is not None or depth_video is not None:
            ...
            # MODIFY: add depth_video and blending_area_mask processing
            inactive = vace_video * (1 - vace_video_mask) + 0 * vace_video_mask
            # reactive = vace_video * vace_video_mask + 0 * (1 - vace_video_mask)
            depth_as_reactive = False
            use_only_blending = False

            # only blending as reactive
            if blending_area_mask is None:
                reactive = vace_video * vace_video_mask + 0 * (1 - vace_video_mask)
            else:
                use_only_blending = True
                blending_area_mask = pipe.preprocess_video(blending_area_mask, min_value=0, max_value=1)
                reactive = vace_video * (1 - blending_area_mask) + 0 * blending_area_mask

            # # blending+depth as reactive
            # if depth_video is not None:
            #     depth_video = pipe.preprocess_video(depth_video)
            #     if blending_area_mask is None:
            #         blending_area_mask = vace_video_mask
            #     else:
            #         blending_area_mask = pipe.preprocess_video(blending_area_mask, min_value=0, max_value=1)
            #     reactive = (vace_video * (1 - blending_area_mask)
            #                 + depth_video * (vace_video_mask - 1 + blending_area_mask)
            #                 + 0 * (1 - vace_video_mask))
            #     depth_as_reactive = True
            # else:
            #     reactive = vace_video * vace_video_mask + 0 * (1 - vace_video_mask)
            ...
            depth_video_latents = None
            # Full Depth
            if depth_video is not None:
                video_for_depth = pipe.preprocess_video(depth_video)
                depth_video_latents = pipe.vae.encode(video_for_depth, device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride).to(dtype=pipe.torch_dtype, device=pipe.device)
                
            # mix Mask
            m = vace_video_mask[0,0]
            b = blending_area_mask[0,0]
            m[b<0.5]= 0.5
            vace_mask_latents = rearrange(m, "T (H P) (W Q) -> 1 (P Q) T H W", P=8, Q=8)
            ...
            if vace_reference_image is None:
                ...
                if depth_video_latents is not None:
                    depth_video_latents = torch.concat(
                        (
                            torch.zeros(
                                (depth_video_latents.shape[0], 16, f, *depth_video_latents.shape[3:]),
                                dtype=depth_video_latents.dtype,
                                device=depth_video_latents.device,
                            ),
                            depth_video_latents,
                        ),
                        dim=2,
            ...
            expected_in = pipe.vace.vace_patch_embedding.in_channels
            depth_mode = getattr(pipe.vace, "depth_mode", "adapter")
            if depth_mode not in ("adapter", "concat"):
                raise ValueError(
                    "Unsupported VACE depth mode: "
                    f"{depth_mode!r}. Expected one of ('adapter', 'concat')."
                )
            if expected_in not in (96, 112):
                raise ValueError(
                    "Unsupported VACE input channels: "
                    f"pipe.vace expects {expected_in} channels. "
                    "Expected one of (96, 112)."
                )
            if depth_mode == "concat":
                if expected_in < 112:
                    raise ValueError(
                        "VACE depth concat mode requires vace_in_dim >= 112. "
                        f"Current in_channels={expected_in}."
                    )
                if depth_video_latents is None:
                    depth_video_latents = torch.zeros(
                        (vace_context.shape[0], 16, *vace_context.shape[2:]),
                        dtype=vace_context.dtype,
                        device=vace_context.device,
                    )
            if vace_context.shape[1] != expected_in:
                raise ValueError(
                    "VACE channel mismatch: "
                    f" pipe.vace expects {expected_in} channels, "
                    f" but vace_context has {vace_context.shape[1]} channels."
                )
                vace_context = torch.concat((vace_context, depth_video_latents), dim=1)
            elif expected_in >= 112:
                raise ValueError(
                    "VACE depth adapter mode requires vace_in_dim < 112. "
                    f"Current in_channels={expected_in}."
                )
            return {"vace_context": vace_context, "vace_depth_latents": depth_video_latents, "vace_scale": vace_scale}
        else:
            return {"vace_context": None, "vace_depth_latents": None, "vace_scale": vace_scale}
...
def model_fn_wan_video(
    ...
    vace_context = None,
    vace_depth_latents = None,  # MODIFY: add vace depth latents input
    vace_scale = 1.0,
    audio_embeds: Optional[torch.Tensor] = None,
    ...
    **kwargs,
    ...
    if sliding_window_size is not None and sliding_window_stride is not None:
        model_kwargs = dict(
            ...
            vace_depth_latents=vace_depth_latents,  #MODIFY: add vace depth latents input
    ...
    if vace_context is not None:
        vace_hints = vace(
            ...
            depth_latents=vace_depth_latents,   # MODIFY: add vace depth latents input
            use_gradient_checkpointing=use_gradient_checkpointing,
            use_gradient_checkpointing_offload=use_gradient_checkpointing_offload
        )
    if tea_cache_update:
