...
def _sdpa_varlen_attention(q, k, v, cu_seqlens, softmax_scale, seq_bounds=None):
    ...
    if len(bounds) == 2:
        return attention_forward(q.transpose(0, 1).unsqueeze(0), k.transpose(0, 1).unsqueeze(0), v.transpose(0, 1).unsqueeze(0), scale=softmax_scale).squeeze(0).transpose(0, 1)
