...
class QwenImage21ResidualUpBlock(nn.Module):
    ...
    def __init__(self, in_dim: int, out_dim: int, num_res_blocks: int, dropout: float = 0.0, temperal_upsample: bool = False, up_flag: bool = False, non_linearity: str = "silu"):
        super().__init__()
        self.in_dim = in_dim
        self.out_dim = out_dim
...
class QwenImage21UpBlock(nn.Module):
    ...
    def __init__(self, in_dim: int, out_dim: int, num_res_blocks: int, dropout: float = 0.0, upsample_mode: str | None = None, non_linearity: str = "silu"):
        super().__init__()
        self.in_dim = in_dim
        self.out_dim = out_dim
...
class QwenImage21VAE(nn.Module):
    def __init__(
        ...
        if len(latents_mean) != z_dim or len(latents_std) != z_dim:
            raise ValueError("latents_mean and latents_std must each contain z_dim values")
        ...
        self.latents_mean = torch.tensor(latents_mean).view(1, z_dim, 1, 1, 1)
        self.latents_std = torch.tensor(latents_std).view(1, z_dim, 1, 1, 1)
        ...
        self.spatial_compression_ratio = 2 ** (len(dim_mult) - 1)
    ...
    def enable_tiling(self, tile_size: int = 256, tile_stride: int = 192) -> None:
        self.use_tiling = True
        self.tile_sample_min_height = tile_size
        self.tile_sample_min_width = tile_size
        self.tile_sample_stride_height = tile_stride
        self.tile_sample_stride_width = tile_stride

    def _configure_tiling(self, tiled: bool, tile_size: int, tile_stride: int) -> None:
        self.use_tiling = tiled
        if tiled:
            self.enable_tiling(tile_size, tile_stride)
    ...
    def encode(self, x: torch.Tensor, tiled: bool = False, tile_size: int = 256, tile_stride: int = 192) -> torch.Tensor:
        self._configure_tiling(tiled, tile_size, tile_stride)
        image_input = x.ndim == 4
        if image_input:
            x = x.unsqueeze(2)
        elif x.ndim != 5:
            raise ValueError(f"Expected a 4D image or 5D video tensor, got shape {tuple(x.shape)}")
        ...
        return latents.squeeze(2) if image_input else latents
    ...
    def decode(self, z: torch.Tensor, tiled: bool = False, tile_size: int = 256, tile_stride: int = 192) -> torch.Tensor:
        self._configure_tiling(tiled, tile_size, tile_stride)
        image_input = z.ndim == 4
        if image_input:
            z = z.unsqueeze(2)
        elif z.ndim != 5:
            raise ValueError(f"Expected a 4D image latent or 5D video latent, got shape {tuple(z.shape)}")
        ...
        return decoded.squeeze(2) if image_input else decoded
    ...
    def tiled_decode(self, z: torch.Tensor) -> torch.Tensor:
        """Decode a batch of latent tensors using overlapping spatial tiles."""
