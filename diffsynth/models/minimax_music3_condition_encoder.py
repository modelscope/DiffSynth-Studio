...
class MiniMaxMusic3ConditionEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.cond_layer_logits = nn.Parameter(torch.zeros(num_condition_layers))
        self.cond_layer_scale = nn.Parameter(torch.ones(1))
        self.latent_conditioners = nn.ModuleList(
            [nn.Conv1d(condition_hidden_dim, out_dim, kernel_size=3, padding=1)]
        )
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        layer_weights = torch.softmax(self.cond_layer_logits, dim=0).to(hidden_states.dtype)
        hidden_states = torch.einsum("blht,l->bht", hidden_states, layer_weights)
        hidden_states = self.cond_layer_scale.to(hidden_states.dtype) * hidden_states
        hidden_states = self.latent_conditioners[0](hidden_states)
        latent_length = max(
            1,
            int(
                num_frames
