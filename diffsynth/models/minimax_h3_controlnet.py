...
class MiniMaxH3ControlNet(nn.Module):
    """VACE-style control branch for MiniMax-H3.

    A parallel stack of blocks runs over the packed sequence with the clean control latents in
    place of the video rows, and emits one zero-gated skip per addressed main layer. Both
    `before_proj` and `after_proj` start at zero, so an untrained branch reproduces the base model
    exactly.

    `control_in_dim` is the channel count of one control row before patchification. The video VAE
    emits 24 latent channels, so a control-only branch uses 24, while an inpaint-capable branch
    widens it to 49 = 24 control + 1 visibility map + 24 masked-video latents. It must match the
    checkpoint, since it sets the `control_patch_proj` shape.

    `control_apply_audio` decides whether the skips reach the audio rows. `False` keeps the
    soundtrack on the base model's path, so the control video guides the picture alone.
    """
