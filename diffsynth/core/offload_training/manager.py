...
class OffloadTrainingManager:
    ...
    def __init__(
        ...
        self._cpu_copies: dict[int, torch.Tensor] = {}

        # Separate trainable vs non-trainable
    ...
    def _load_param_to_gpu(self, param, device):
        ...
        if pid in self._cpu_copies:
            param.data = self._cpu_copies[pid].to(device, non_blocking=True)
        else:
    ...
    def _offload_param_to_cpu(self, param):
        ...
        pid = id(param)
        if pid in self._cpu_copies:
            param.data = self._cpu_copies[pid]
        else:
