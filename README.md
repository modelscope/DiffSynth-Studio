...
- **May 15, 2026** Added **CPU Offload Training** support. By moving model weights layer-by-layer between CPU and GPU, it significantly reduces GPU VRAM usage during training, enabling LoRA training of large models even on consumer-grade GPUs, compatible with all models. Simply add `--cpu_offload` to your training command to enable. For details, see the [documentation](/docs/en/Training/Offload_Training.md).
