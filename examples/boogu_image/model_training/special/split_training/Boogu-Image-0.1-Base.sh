...
accelerate launch examples/boogu_image/model_training/train.py \
  ...
  --output_path ./models/train/Boogu-Image-0.1-Base_split_cache \
  --lora_base_model dit \
  ...
  --offload_models 'Boogu/Boogu-Image-0.1-Base:transformer/*.safetensors' \
  --task sft:data_process

# Stage 2: train LoRA from the cached dataset.
accelerate launch examples/boogu_image/model_training/train.py \
  --dataset_base_path ./models/train/Boogu-Image-0.1-Base_split_cache \
  --max_pixels 1048576 \
  --dataset_repeat 50 \
  --model_id_with_origin_paths 'Boogu/Boogu-Image-0.1-Base:transformer/*.safetensors,Boogu/Boogu-Image-0.1-Base:mllm/*.safetensors,Boogu/Boogu-Image-0.1-Base:vae/*.safetensors' \
  --learning_rate 1e-4 \
  --num_epochs 5 \
  --remove_prefix_in_ckpt pipe.dit. \
  --output_path ./models/train/Boogu-Image-0.1-Base_split \
  --lora_base_model dit \
  --lora_target_modules to_q,to_k,to_v,to_out.0,img_to_q,img_to_k,img_to_v,img_out,instruct_to_q,instruct_to_k,instruct_to_v,instruct_out \
  --lora_rank 32 \
  --use_gradient_checkpointing \
  ...
  --find_unused_parameters \
  --data_file_keys image \
  --offload_models 'Boogu/Boogu-Image-0.1-Base:mllm/*.safetensors,Boogu/Boogu-Image-0.1-Base:vae/*.safetensors' \
  --task sft:train
