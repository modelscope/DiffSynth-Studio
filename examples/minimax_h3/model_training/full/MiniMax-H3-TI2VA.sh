modelscope download --dataset DiffSynth-Studio/diffsynth_example_dataset --include "minimax_h3/MiniMax-H3-T2VA/*" --local_dir ./data/diffsynth_example_dataset

# ===== 1/2  T2VA (text -> video + audio), splited =====
accelerate launch examples/minimax_h3/model_training/train.py \
  --dataset_base_path data/diffsynth_example_dataset/minimax_h3/MiniMax-H3-T2VA \
  --dataset_metadata_path data/diffsynth_example_dataset/minimax_h3/MiniMax-H3-T2VA/metadata.csv \
...
# ===== 2/2  FL2VA (first + last frame -> video + audio), splited =====
# `input_image` / `end_image` take the first and last frame of the training video itself, so the
# dataset needs no extra column; the training module turns them into keyframe_indices [0, -1].
accelerate launch examples/minimax_h3/model_training/train.py \
  --dataset_base_path data/diffsynth_example_dataset/minimax_h3/MiniMax-H3-T2VA \
  --dataset_metadata_path data/diffsynth_example_dataset/minimax_h3/MiniMax-H3-T2VA/metadata.csv \
  --data_file_keys "video,input_audio" \
