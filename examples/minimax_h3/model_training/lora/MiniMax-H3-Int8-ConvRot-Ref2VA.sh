...
accelerate launch examples/minimax_h3/model_training/train.py \
  ...
  --output_path "./models/train/MiniMax-H3-Ref2VA-int8-convrot-cache" \
  --lora_base_model "dit" \
...
accelerate launch examples/minimax_h3/model_training/train.py \
  --dataset_base_path "./models/train/MiniMax-H3-Ref2VA-int8-convrot-cache" \
  --num_frames 124 \
  --dataset_repeat 100 \
  ...
  --num_epochs 5 \
  --remove_prefix_in_ckpt "pipe.dit." \
  --output_path "./models/train/MiniMax-H3-Ref2VA-int8-convrot" \
  --lora_base_model "dit" \
  --lora_target_modules "attn.qkv_proj,attn.out_proj,mlp.fc1,mlp.fc2" \
  --lora_rank 32 \
  --use_gradient_checkpointing \
