...
pipe.load_lora(pipe.dit, "models/train/MiniMax-H3-T2VA-pruned-nf4/epoch-4.safetensors")
video, audio = pipe(
    prompt=prompt,
...
write_video_audio(
    video=video, audio=audio, output_path="minimax_h3_t2va_lora_pruned_nf4.mp4",
    fps=24, audio_sample_rate=pipe.audio_vae.sample_rate,
)
print("saved minimax_h3_t2va_lora_pruned_nf4.mp4", "frames:", len(video), "audio:", tuple(audio.shape))

frames = VideoData(f"{dataset_base_path}/video.mp4", height=height, width=width).raw_data()
pipe.clear_lora()
pipe.load_lora(pipe.dit, "models/train/MiniMax-H3-FL2VA-pruned-nf4/epoch-4.safetensors")
video, audio = pipe(
    prompt=prompt,
    height=height, width=width, num_frames=num_frames,
    num_inference_steps=50, seed=0,
...
write_video_audio(
    video=video, audio=audio, output_path="minimax_h3_fl2va_lora_pruned_nf4.mp4",
    fps=24, audio_sample_rate=pipe.audio_vae.sample_rate,
)
print("saved minimax_h3_fl2va_lora_pruned_nf4.mp4", "frames:", len(video), "audio:", tuple(audio.shape))
