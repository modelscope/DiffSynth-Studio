...
pipe = MiniMaxH3Pipeline.from_pretrained(
    ...
    model_configs=[
        ...
        ModelConfig(model_id="Comfy-Org/MiniMax-H3", origin_file_pattern="diffusion_models/minimax_h3_fl2va_pruned_fp8_scaled.safetensors", **vram_config),
...
dataset_snapshot_download(
    ...
    allow_file_pattern="minimax_h3/MiniMax-H3-Pruned-FL2VA/*",
)
...
pipe.load_lora(pipe.dit, "models/train/MiniMax-H3-T2VA-pruned-fp8/epoch-4.safetensors")
video, audio = pipe(
    prompt=prompt,
...
write_video_audio(
    video=video, audio=audio, output_path="minimax_h3_t2va_lora_pruned_fp8.mp4",
    fps=24, audio_sample_rate=pipe.audio_vae.sample_rate,
)
print("saved minimax_h3_t2va_lora_pruned_fp8.mp4", "frames:", len(video), "audio:", tuple(audio.shape))

frames = VideoData(f"{dataset_base_path}/video.mp4", height=height, width=width).raw_data()
pipe.clear_lora()
pipe.load_lora(pipe.dit, "models/train/MiniMax-H3-FL2VA-pruned-fp8/epoch-4.safetensors")
video, audio = pipe(
    prompt=prompt,
    height=height, width=width, num_frames=num_frames,
    num_inference_steps=50, seed=0,
...
write_video_audio(
    video=video, audio=audio, output_path="minimax_h3_fl2va_lora_pruned_fp8.mp4",
    fps=24, audio_sample_rate=pipe.audio_vae.sample_rate,
)
print("saved minimax_h3_fl2va_lora_pruned_fp8.mp4", "frames:", len(video), "audio:", tuple(audio.shape))
