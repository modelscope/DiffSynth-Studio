...
pipe = MiniMaxH3Pipeline.from_pretrained(
    ...
    model_configs=[
        ...
        ModelConfig(model_id="Comfy-Org/MiniMax-H3", origin_file_pattern="diffusion_models/minimax_h3_fl2va_pruned_bf16.safetensors", **vram_config),
...
pipe.load_lora(pipe.dit, "models/train/MiniMax-H3-T2VA-split/epoch-4.safetensors")
video, audio = pipe(
    prompt=prompt,
...
write_video_audio(
    video=video, audio=audio, output_path="minimax_h3_t2va_lora.mp4",
...
pipe.clear_lora()
pipe.load_lora(pipe.dit, "models/train/MiniMax-H3-FL2VA-split/epoch-4.safetensors")
video, audio = pipe(
    prompt=prompt,
    height=height, width=width, num_frames=num_frames,
    num_inference_steps=50, seed=0,
...
write_video_audio(
    video=video, audio=audio, output_path="minimax_h3_fl2va_lora.mp4",
    fps=24, audio_sample_rate=pipe.audio_vae.sample_rate,
)
print("saved minimax_h3_fl2va_lora.mp4", "frames:", len(video), "audio:", tuple(audio.shape))
