...
# Ref2AV uses the Ref2VA transformer partition (same model_id, different weight files).
...
# Target canvas: 768 (height) × 1344 (width). Pre-resize refs to target size so
# the pipeline's strict target-library short_edge=2048 scaling doesn't blow up
# GPU memory on our single 80GB card. (The target library ran on multi-GPU with
# sequence parallelism.)
TARGET_H, TARGET_W = 768, 1344

# Example 1: image + video reference
...
ref_video_raw = load_video_frames("assets_minimax/ref2av/example1/ref1.mov", max_frames=121)
ref_video_frames = [
...
video, audio = pipe(
    prompt=prompt,
    height=TARGET_H, width=TARGET_W, num_frames=121,
