...
# Splited Training (debug: 1 epoch, dataset_repeat 1)
accelerate launch examples/ltx2/model_training/train.py \
  ...
  --output_path "./models/train/LTX2.5-T2AV_lora-splited-cache-test" \
  --lora_base_model "dit" \
...
accelerate launch examples/ltx2/model_training/train.py \
  --dataset_base_path ./models/train/LTX2.5-T2AV_lora-splited-cache-test \
  --data_file_keys "video,input_audio" \
  --extra_inputs "input_audio" \
  --height 512 \
  --width 768 \
  ...
  --num_epochs 1 \
  --remove_prefix_in_ckpt "pipe.dit." \
  --output_path "./models/train/LTX2.5-T2AV_lora-test" \
  --lora_base_model "dit" \
  --lora_target_modules "to_k,to_q,to_v,to_out.0" \
  --lora_rank 32 \
  --use_gradient_checkpointing \
