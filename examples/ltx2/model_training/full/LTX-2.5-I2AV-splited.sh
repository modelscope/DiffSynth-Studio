...
accelerate launch examples/ltx2/model_training/train.py \
  ...
  --output_path "./models/train/LTX2.5-I2AV-full-splited-cache" \
...
accelerate launch --config_file examples/ltx2/model_training/full/accelerate_config_zero2offload.yaml examples/ltx2/model_training/train.py \
  --dataset_base_path ./models/train/LTX2.5-I2AV-full-splited-cache \
  --data_file_keys "video,input_audio" \
  --extra_inputs "input_audio,input_image" \
  --height 512 \
  --width 768 \
  ...
  --num_epochs 2 \
  --remove_prefix_in_ckpt "pipe.dit." \
  --output_path "./models/train/LTX2.5-I2AV-full" \
  --trainable_models "dit" \
  --use_gradient_checkpointing \
