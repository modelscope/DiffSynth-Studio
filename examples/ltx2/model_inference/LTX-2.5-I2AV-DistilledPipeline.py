...
# The example frames come from the shared sample dataset, so reuse their paired prompt.
...
write_video_audio_ltx2(
    video=video,
    audio=audio,
    output_path="ltx2.5_distilled_i2av_first.mp4",
    fps=24,
    audio_sample_rate=pipe.audio_vocoder.output_sampling_rate,
)

# Multi-keyframe interpolation: any frame indexes within num_frames are supported.
video, audio = pipe(
    prompt=prompt,
    negative_prompt=negative_prompt,
    seed=43,
...
write_video_audio_ltx2(
    video=video,
    audio=audio,
    output_path="ltx2.5_distilled_i2av_keyframes.mp4",
    fps=24,
    audio_sample_rate=pipe.audio_vocoder.output_sampling_rate,
)
