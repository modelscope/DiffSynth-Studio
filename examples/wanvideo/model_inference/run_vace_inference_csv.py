...
def main():
    ...
    p.add_argument(
        "--depth_mode",
        default=None,
        choices=["depth_adapter", "adapter", "concat"],
        help=(
            "VACE depth mode. Use 'depth_adapter' (alias: 'adapter') or 'concat'. "
            "If omitted, infer from vace_in_dim: 96 -> adapter, >=112 -> concat."
        ),
    )
    ...
    if args.depth_mode is None:
        depth_mode = "adapter" if args.vace_in_dim == 96 else "concat"
    else:
        depth_mode = "adapter" if args.depth_mode == "depth_adapter" else args.depth_mode

    if depth_mode == "adapter" and args.vace_in_dim != 96:
        print(
            f"[WARN] depth_mode=adapter usually expects vace_in_dim=96, got {args.vace_in_dim}."
        )
    if depth_mode == "concat" and args.vace_in_dim < 112:
        print(
            f"[WARN] depth_mode=concat usually expects vace_in_dim>=112, got {args.vace_in_dim}."
        )

    os.environ["DIFFSYNTH_WAN_VACE_IN_DIM"] = str(args.vace_in_dim)
    os.environ["DIFFSYNTH_WAN_VACE_DEPTH_MODE"] = depth_mode
    print(f"[Config] vace_in_dim={args.vace_in_dim}, depth_mode={depth_mode}")
