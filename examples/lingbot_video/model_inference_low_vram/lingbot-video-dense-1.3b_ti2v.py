...
# Low-VRAM image-to-video (TI2V). offload_dtype / offload_device on each ModelConfig turn on
# VRAM management: weights stay on CPU in fp8 and stream to the GPU layer-by-layer, computed
# in bf16. vram_limit only caps resident VRAM once offloading is enabled by those two fields.
# The TI2V delta over t2v is just `input_image` — the condition-frame VAE encode / text-encoder
# vision pass run under the same offloading, so peak VRAM matches the low-VRAM t2v run.
vram_config = {
