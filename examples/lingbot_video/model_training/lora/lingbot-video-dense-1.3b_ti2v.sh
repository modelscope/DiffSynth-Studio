...
# `--first_frame_as_condition`, which conditions each clip on its OWN first frame: the frame
# is VAE-encoded to a clean latent pinned into the first temporal slot (and fed to the
# Qwen3-VL text encoder), and excluded from the flow-matching loss, so the LoRA learns to
# animate frames 2..N from frame 1. Dense-1.3B reuses the same T2V weights — no separate i2v
# checkpoint. Reuses the shared example dataset (plain `video` + `prompt`); no condition
# column is required. If your dataset instead ships a distinct condition frame, drop this
# flag and pass it via `--extra_inputs input_image` (adding that column to --data_file_keys).
