...
accelerate launch examples/lingbot_video/model_training/train.py \
  ...
  --output_path ./models/train/lingbot-video-dense-1.3b_t2v_split_cache \
  --lora_base_model dit \
  --lora_target_modules to_q,to_k,to_v,to_out \
  --lora_rank 32 \
  --use_gradient_checkpointing \
  --offload_models Robbyant/lingbot-video-dense-1.3b:transformer/diffusion_pytorch_model.safetensors \
  --task sft:data_process

# Stage 2: train LoRA from the cached dataset.
accelerate launch examples/lingbot_video/model_training/train.py \
  --dataset_base_path ./models/train/lingbot-video-dense-1.3b_t2v_split_cache \
  --data_file_keys video \
  --height 480 \
  --width 832 \
  --num_frames 81 \
  ...
  --num_epochs 5 \
  --remove_prefix_in_ckpt pipe.dit. \
  --output_path ./models/train/lingbot-video-dense-1.3b_t2v_split \
  --lora_base_model dit \
  --lora_target_modules to_q,to_k,to_v,to_out \
  --lora_rank 32 \
  --use_gradient_checkpointing \
  --offload_models 'Qwen/Qwen3-VL-4B-Instruct:*.safetensors,Robbyant/lingbot-video-dense-1.3b:vae/diffusion_pytorch_model.safetensors' \
  --task sft:train
