...
from diffsynth.models.sensenova_u1_common import PATCH_SIZE
from diffsynth.pipelines.sensenova_u1_image import SenseNovaU1ImagePipeline, ModelConfig
from diffsynth.diffusion import *
os.environ["TOKENIZERS_PARALLELISM"] = "false"


class SenseNovaU1ImageTrainingModule(DiffusionTrainingModule):
    def __init__(
        self,
        ...
        tokenizer_config = self.parse_path_or_model_id(tokenizer_config, default_value=ModelConfig(model_id="SenseNova/SenseNova-U1.5-8B-MoT", origin_file_pattern="./"))
        self.pipe = SenseNovaU1ImagePipeline.from_pretrained(torch_dtype=torch.bfloat16, device=device, model_configs=model_configs, tokenizer_config=tokenizer_config)
    ...
    def get_pipeline_inputs(self, data):
        ...
        inputs_shared = {
            ...
            "height": image.size[1],
            "width": image.size[0],
            # cfg_scale 1 makes the unit runner skip the negative branch, so each step runs the
            # conditioning prefix through the 8B understanding branch once instead of twice.
            "cfg_scale": 1,
...
def sensenova_u1_image_parser():
    parser = argparse.ArgumentParser(description="SenseNova-U1 image training.")
    parser = add_general_config(parser)
    parser = add_image_size_config(parser)
    parser.add_argument("--tokenizer_config", type=str, default=None, help="Path to tokenizer config.")
    parser.add_argument("--initialize_model_on_cpu", default=False, action="store_true", help="Whether to initialize models on CPU.")
    return parser


if __name__ == "__main__":
    parser = sensenova_u1_image_parser()
    ...
    model = SenseNovaU1ImageTrainingModule(
