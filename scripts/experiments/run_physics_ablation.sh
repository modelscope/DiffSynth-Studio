...
python scripts/validation/validate_lora.py \
    ...
    --seed 42 --steps 28
python scripts/metrics/run_metrics.py "${OUTPUT_BASE}/baseline" --save --output-file metrics.txt

# 2. Latent-DPS with baseline checkpoint
echo ""
echo "--- (B) Baseline + Latent-DPS ---"
for ETA in 0.01 0.05 0.1 0.5 1.0; do
    echo "  eta=${ETA}..."
    python scripts/validation/validate_dps.py \
        --lora_checkpoint "${LORA_CHECKPOINT_BASELINE}" \
        --metadata_csv "${METADATA_CSV}" \
    ...
    python scripts/metrics/run_metrics.py "${OUTPUT_BASE}/dps_eta${ETA}" --save --output-file metrics.txt
done
...
if [ -n "$LORA_CHECKPOINT_CONSISTENCY" ]; then
    ...
    python scripts/validation/validate_lora.py \
        ...
        --seed 42 --steps 28
    python scripts/metrics/run_metrics.py "${OUTPUT_BASE}/consistency" --save --output-file metrics.txt
    ...
    python scripts/validation/validate_dps.py \
        ...
        --dps_schedule linear_decay
    python scripts/metrics/run_metrics.py "${OUTPUT_BASE}/consistency_dps" --save --output-file metrics.txt
fi
...
echo ""
echo "Results:"
for DIR in baseline dps_eta0.01 dps_eta0.05 dps_eta0.1 dps_eta0.5 dps_eta1.0 consistency consistency_dps; do
