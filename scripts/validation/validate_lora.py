...
def main():
    ...
    parser.add_argument("--lora_checkpoint", type=str, default=None, help="Path to LoRA checkpoint (omit for no-LoRA baseline)")
    ...
    parser.add_argument("--dataset_base", type=str, default=None, help="Base directory for resolving CSV paths (default: CSV parent dir)")
    ...
    if args.lora_checkpoint is not None:
        ...
        print("No LoRA checkpoint — running with frozen pretrained ControlNet + DiT")
    
    # Run inference
    print("Running inference on validation set...")
    csv_base = Path(args.dataset_base) if args.dataset_base else Path(args.metadata_csv).parent
    ...
    for idx, sample in enumerate(tqdm(samples, desc="Generating")):
        ...
        control_img = load_spad_image(input_path)
